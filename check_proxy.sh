...
# следующему по списку хосту. Последнее сделано для того, чтобы избежать нештатных ситуаций
# "подвисания" программы.
# Исключаемые порты из проверки: 45554, 1080, 10200, 9999, 8081, 9000
#====================================================================================================

BAD_FILE=71 #Код принудительного завершения работы скрипта
#========================================================================================
# Проверка на наличия файла со списком прокси, если его нет, то скачивается новый список,
# либо, при наличии списка рабочих прокси, происходит их перепроверка
#========================================================================================
plf=proxy.list # Входящий файл со списком прокси для проверки
wplf=work_proxy.list # Исходящий файл со списком работающих прокси
if [ ! -r "$plf" ]
  then
  ...
  if [ "$n_plf" = "0" ]
     ...
     if [ ! -r "$wplf" ]
        then
           ...
           if [ "$n_plf" = "0" ]
              then
                 echo "Отсутствует файл рабочих прокси. Файл с входящим списком прокси пустой. Программа завершается..."
                 exit $BAD_FILE
           fi
        else
        ...
        if [ "$n_wplf" = "0" ]
           then
              ...
              if [ "$n_plf" = "0" ]
                 then
                    echo "Копируем рабочие прокси во входящий файл. Для перепроверки."
                    cp $wplf $plf
              fi
        fi
     fi
